...
const {
  resizeImage, readFile, s3Upload, s3DeleteObject
...
router.post('/deleteImageAWS', async (req, res, next) => {
  try {
    const data = req.body;
    const { id } = data;
    ...
    const PhotoInfo = await Photo.findOne({
      where: { PHO_ID: id },
      attributes: ['PHO_FILE_KEY']
    });

    const { PHO_FILE_KEY } = PhotoInfo;

    await s3DeleteObject(PHO_FILE_KEY);
    await Photo.destroy({ where: { PHO_ID: id } });

    return res.status(200).json({ PHO_FILE_KEY });
  } catch (err) {
    return res.status(400).json({ uploaded: false, error: { message: err.message } });
  }
});
